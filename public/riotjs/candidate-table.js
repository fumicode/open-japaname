...
riot.tag2('candidate-table', '<div class="syllablesTable"> <div class="syllablesTable__item {syllable == usingSyllable ? \'syllablesTable__item--using\':\'\'}" each="{syllable in selectedSyllables}" onclick="{parent.syllableSelected}" ontouchstart="{parent.syllableSelected}"> <h3 class="syllablesTable__head">{syllable.romaji} - {syllable.kana}</h3> <div class="syllablesTable__kanjis"> <table class="kanjisList"> <tr class="kanjisList__row"> <td class="kanjisList__item" each="{other_syllable_path in syllable.siblings}" onclick="{parent.parent.changeSyllable}"> <div class="otherSyllableCell"> <ul> <li class="otherSyllableCell__part" each="{other_syllable in other_syllable_path}">{other_syllable.romaji}</li> </ul> <div class="otherSyllableCell__board"> <p>Change syllable separation to</p> <ul> <li each="{other_syllable in other_syllable_path}">{other_syllable.romaji}<span each="{kanji in other_syllable.atejis.slice(0,3)}">{kanji.kanji}</span></li> </ul> </div> </div> </td> <td class="kanjisList__item" each="{kanji in syllable.atejis}"> <div class="kanjiCell {kanji == syllable.usingKanji ? \'kanjiCell--using\':\'\'}" onclick="{parent.parent.kanjiSelected}"> <div class="kanjiCell__char">{kanji.kanji}</div> <div class="kanjiCell__meanings">{kanji.meanings.join(⁗\\n⁗)}</div> </div> </td> </tr> </table> </div> </div> </div>', '', '', function(opts) {
    this_tag = this;
