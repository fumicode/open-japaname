...
riot.tag2('kanji-detail', '<div class="kanjiDetail {opened ? \'kanjiDetail--opened\':\'\'}"> <div class="kanjiDetail__inner"> <div class="kanjiDetail__leftColumn"> <div class="kanjiDetail__bigKanji"> <span class="kanjiDetail__bigKanjiInner">{thekanji} </span></div> </div> <div class="kanjiDetail__centerColumn"> <div class="kanjiDetail__sounds"> <virtual each="{sound in romaji_sounds}">[{sound}]</virtual> </div> <div class="kanjiDetail__meaning">{meaning}</div> </div> <div class="kanjiDetail__rightColumn"> <form onsubmit="{addComment}"> <input type="text" ref="comment_input" value="{kanji.comment}"> <input type="submit" ref="コ メ ン ト を 追 加"> </form> </div> </div> </div>', '', '', function(opts) {
    ...
    this.addComment = function(e){
      ...
      this.kanji.comment = comment_text;
      e.preventDefault();
    }.bind(this)
